...
void loop(void)
{
  ...
  Serial.println("\rEnter URL:\r\n");
  // Wait for user input before proceeding

  // a key was pressed
  Serial.println("Waiting for a URL...");
  while (!Serial.available());
  while (Serial.available()) {
    read_url = Serial.readStringUntil(newLine);
  }
  Serial.print("URL: ");
    ...
    else
    {
      ...
      else
      {
        ...
        else
        {
          ...
          //int url_size = sizeof(read_url);
          //char * url;
          //read_url.toCharArray(url, url_size);
          char * url = const_cast<char*>(read_url.c_str());
